...
class BannerController extends ApiController
{
    public function index($siteName){
        $data = DB::select("SELECT link, target, rel, image, alt, width, height, type, content, (SELECT banner_site.`slug` FROM banner_site WHERE `type` = 'position' AND id = id_position) AS vitri
