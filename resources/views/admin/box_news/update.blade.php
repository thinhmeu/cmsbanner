...
@section('content')
            ...
            <div class="fade-in">
                <form method="post" action="">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="card">
                                <div class="card-header"><strong>{{!empty($oneItem) ? 'Chỉnh sửa' : 'Thêm mới'}} Box News</strong></div>
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label>Tên Box News</label>
                                                <input class="form-control" name="name" value="{{!empty($oneItem->name) ? $oneItem->name : ''}}" type="text" placeholder="Tên Box News" required>
                                                    ...
                                                    <select class="form-control" name="category_id">
                                                        ...
                                                        @foreach($categoryTree as $item)
                                                            <option value="{{$item['id']}}" data-title="{{$item['title']}}"
                                                                    data-url="/{{ $item['slug'] }}/">{{$item['title']}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
    ...
    <script defer src="/admin/js/nestable/box_news.js?{{time()}}"></script>
@endsection
